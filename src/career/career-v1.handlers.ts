  ...
  private async run(operation: WorkflowOperation, signal: AbortSignal) {
    ...
    return this.complete(operation, signal, async (manager) => {
      ...
      if (mode !== RepositoryMode.ManualGreenfield) {
        ...
        binding = { mode, installationId, githubRepositoryId: member.githubRepositoryId, repositoryName: member.fullName, repositoryPrivate: member.private, bindingVersion: 1, pullNumber: 42, expectedHeadSha: 'a'.repeat(40) };
      }
