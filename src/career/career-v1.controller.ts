import { BadRequestException, Body, Controller, Get, Header, Headers, HttpCode, Param, ParseUUIDPipe, Post, UseGuards } from '@nestjs/common';
...
export class CareerV1Controller {
  ...
  @Post('target-imports') @HttpCode(202) @Header('Retry-After', WORKFLOW_RETRY_AFTER_SECONDS) targetImport(@CurrentUser() user: AuthUser, @Headers('idempotency-key') key: string, @Body() body: TargetImportDto) { return this.service.targetImport(user.id, requireKey(key), body as unknown as Record<string, unknown>); }
  @Post('profile-snapshot-operations/github') @HttpCode(202) @Header('Retry-After', WORKFLOW_RETRY_AFTER_SECONDS) profileOperation(@CurrentUser() user: AuthUser, @Headers('idempotency-key') key: string, @Body() body: ProfileSnapshotOperationDto) { return this.service.profileOperation(user.id, requireKey(key), body as unknown as Record<string, unknown>); }
...
export class ProjectRunOperationsController {
  constructor(private readonly service: CareerV1Service) {}
  @Post() @HttpCode(202) @Header('Retry-After', WORKFLOW_RETRY_AFTER_SECONDS) create(@CurrentUser() user: AuthUser, @Headers('idempotency-key') key: string, @Body() body: CreateProjectRunOperationDto) { return this.service.projectRunOperation(user.id, requireKey(key), body as unknown as Record<string, unknown>); }
}
